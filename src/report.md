...
Разница от порядка команд на разрешение и запрет echo reply заключается в том, что применена будет запрещающая команда, даже если после нее идет разрешающая (но не наоборот)
## 4.2. Утилита nmap
